...
func (oq *OrganizationQuery) QueryApplication() *ApplicationQuery {
	query := (&ApplicationClient{config: oq.config}).Query()
...
func (oq *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withApplication: oq.withApplication.Clone(),
...
func (oq *OrganizationQuery) WithApplication(opts ...func(*ApplicationQuery)) *OrganizationQuery {
	query := (&ApplicationClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withApplication = query
	return oq
}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withApplication; query != nil {
		if err := oq.loadApplication(ctx, query, nodes, nil,
			func(n *Organization, e *Application) { n.Edges.Application = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrganizationQuery) loadApplication(ctx context.Context, query *ApplicationQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Application)) error {
...
func (oq *OrganizationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withApplication != nil {
			_spec.Node.AddColumnOnce(organization.FieldApplicationID)
		}
	}
