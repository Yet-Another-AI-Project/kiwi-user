...
type OrganizationUserQuery struct {
	config
	ctx              *QueryContext
	order            []organizationuser.OrderOption
	inters           []Interceptor
...
func (ouq *OrganizationUserQuery) Order(o ...organizationuser.OrderOption) *OrganizationUserQuery {
...
func (ouq *OrganizationUserQuery) QueryRole() *RoleQuery {
	query := (&RoleClient{config: ouq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := ouq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (ouq *OrganizationUserQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (ouq *OrganizationUserQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (ouq *OrganizationUserQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (ouq *OrganizationUserQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (ouq *OrganizationUserQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (ouq *OrganizationUserQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := ouq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (ouq *OrganizationUserQuery) Clone() *OrganizationUserQuery {
	...
	return &OrganizationUserQuery{
		config:           ouq.config,
		ctx:              ouq.ctx.Clone(),
		order:            append([]organizationuser.OrderOption{}, ouq.order...),
...
func (ouq *OrganizationUserQuery) WithRole(opts ...func(*RoleQuery)) *OrganizationUserQuery {
	query := (&RoleClient{config: ouq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(organizationuser.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.OrganizationUser.Query().
//		Select(organizationuser.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (ouq *OrganizationUserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrganizationUser, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, organizationuser.ForeignKeys...)
	}
	...
	if query := ouq.withRole; query != nil {
		if err := ouq.loadRole(ctx, query, nodes, nil,
			func(n *OrganizationUser, e *Role) { n.Edges.Role = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (ouq *OrganizationUserQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*OrganizationUser, init func(*OrganizationUser), assign func(*OrganizationUser, *Organization)) error {
	...
	nodeids := make(map[uuid.UUID][]*OrganizationUser)
	for i := range nodes {
...
func (ouq *OrganizationUserQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*OrganizationUser, init func(*OrganizationUser), assign func(*OrganizationUser, *User)) error {
	...
	nodeids := make(map[string][]*OrganizationUser)
	for i := range nodes {
		fk := nodes[i].UserID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
...
func (ouq *OrganizationUserQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*OrganizationUser, init func(*OrganizationUser), assign func(*OrganizationUser, *Role)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*OrganizationUser)
	for i := range nodes {
		if nodes[i].organization_user_role == nil {
			continue
		}
		fk := *nodes[i].organization_user_role
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "organization_user_role" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (ouq *OrganizationUserQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(organizationuser.Table, organizationuser.Columns, sqlgraph.NewFieldSpec(organizationuser.FieldID, field.TypeUUID))
	_spec.From = ouq.sql
	...
	if fields := ouq.ctx.Fields; len(fields) > 0 {
		...
		if ouq.withOrganization != nil {
			_spec.Node.AddColumnOnce(organizationuser.FieldOrganizationID)
		}
		if ouq.withUser != nil {
			_spec.Node.AddColumnOnce(organizationuser.FieldUserID)
		}
	}
...
func (ouq *OrganizationUserQuery) ForUpdate(opts ...sql.LockOption) *OrganizationUserQuery {
...
func (ouq *OrganizationUserQuery) ForShare(opts ...sql.LockOption) *OrganizationUserQuery {
	if ouq.driver.Dialect() == dialect.Postgres {
		ouq.Unique(false)
	}
	ouq.modifiers = append(ouq.modifiers, func(s *sql.Selector) {
