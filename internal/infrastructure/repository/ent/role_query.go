...
func (rq *RoleQuery) QueryScopes() *ScopeQuery {
	query := (&ScopeClient{config: rq.config}).Query()
...
func (rq *RoleQuery) QueryApplication() *ApplicationQuery {
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withScopes:      rq.withScopes.Clone(),
		withApplication: rq.withApplication.Clone(),
...
func (rq *RoleQuery) WithScopes(opts ...func(*ScopeQuery)) *RoleQuery {
	query := (&ScopeClient{config: rq.config}).Query()
...
func (rq *RoleQuery) WithApplication(opts ...func(*ApplicationQuery)) *RoleQuery {
	query := (&ApplicationClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withApplication = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withScopes; query != nil {
		if err := rq.loadScopes(ctx, query, nodes,
			func(n *Role) { n.Edges.Scopes = []*Scope{} },
			func(n *Role, e *Scope) { n.Edges.Scopes = append(n.Edges.Scopes, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withApplication; query != nil {
		if err := rq.loadApplication(ctx, query, nodes, nil,
			func(n *Role, e *Application) { n.Edges.Application = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadScopes(ctx context.Context, query *ScopeQuery, nodes []*Role, init func(*Role), assign func(*Role, *Scope)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(scope.FieldRoleID)
	}
	query.Where(predicate.Scope(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.ScopesColumn), fks...))
	}))
...
func (rq *RoleQuery) loadApplication(ctx context.Context, query *ApplicationQuery, nodes []*Role, init func(*Role), assign func(*Role, *Application)) error {
...
func (rq *RoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withApplication != nil {
			_spec.Node.AddColumnOnce(role.FieldApplicationID)
		}
	}
